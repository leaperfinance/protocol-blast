...
contract Comptroller is ComptrollerV9Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event NewSupplyCap(CToken indexed mToken, uint newSupplyCap);
     ...
     */
    function enterMarkets(address[] memory cTokens) virtual override public returns (uint[] memory) {
     ...
     */
    function exitMarket(address cTokenAddress) virtual override external returns (uint) {
     ...
     */
    function mintAllowed(address cToken, address minter, uint mintAmount) virtual override external returns (uint) {
        ...
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cToken, minter);
     ...
     */
    function mintVerify(address cToken, address minter, uint actualMintAmount, uint mintTokens) virtual override external {
        // Shh - currently unused
        cToken;
        minter;
        actualMintAmount;
     ...
     */
    function redeemAllowed(address cToken, address redeemer, uint redeemTokens) virtual override external returns (uint) {
        ...
        // Keep the flywheel moving
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cToken, redeemer);
     ...
     */
    function redeemVerify(address cToken, address redeemer, uint redeemAmount, uint redeemTokens) virtual override external {
        // Shh - currently unused
        cToken;
        redeemer;
     ...
     */
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) virtual override external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!borrowGuardianPaused[cToken], "borrow is paused");
        ...
        RewardDistributor(rewardDistributor).updateAndDistributeBorrowerRewardsForToken(cToken, borrower, borrowIndex);
     ...
     */
    function borrowVerify(address cToken, address borrower, uint borrowAmount) virtual override external {
        // Shh - currently unused
        cToken;
        borrower;
        borrowAmount;
    ...
    function repayBorrowAllowed(
        ...
        address payer,
        address borrower,
        uint repayAmount) virtual override external returns (uint) {
        // Shh - currently unused
        payer;
        borrower;
        repayAmount;
    ...
    function repayBorrowVerify(
        ...
        uint borrowerIndex) virtual override external {
        // Shh - currently unused
        cToken;
        payer;
        borrower;
    ...
    function liquidateBorrowVerify(
        ...
        address borrower,
        uint actualRepayAmount,
        uint seizeTokens) virtual override external {
        // Shh - currently unused
        cTokenBorrowed;
        cTokenCollateral;
        liquidator;
    ...
    function seizeAllowed(
        ...
        address liquidator,
        address borrower,
        uint seizeTokens) virtual override external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        // Keep the flywheel moving
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cTokenCollateral, borrower);
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cTokenCollateral, liquidator);
     ...
     */
    function transferAllowed(address cToken, address src, address dst, uint transferTokens) virtual override external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        // Keep the flywheel moving
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cToken, src);
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(cToken, dst);
     ...
     * @param transferTokens The number of cTokens to transfer
     */
    function transferVerify(address cToken, address src, address dst, uint transferTokens) virtual override external {
        // Shh - currently unused
        cToken;
        src;
        dst;
     ...
     */
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) virtual override external view returns (uint, uint) {
    ...
    function _setRewardDistributor(address payable newRewardDistributor) public returns (uint) {
        ...
        (bool success, ) = newRewardDistributor.call(abi.encodeWithSignature("initialize()", 0));
        if (!success) {
    ...
    function _initializeMarket(address cToken) internal {
        uint32 blockNumber = safe32(getBlockTimestamp(), "block number exceeds 32 bits");
    ...
    /**
      ...
      */
    function _setMarketSupplyCaps(CToken[] calldata mTokens, uint[] calldata newSupplyCaps) external {
