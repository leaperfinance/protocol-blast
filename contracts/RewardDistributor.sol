...
contract RewardDistributorStorage {
    ...
    mapping(uint8 => mapping(address => mapping(address => uint256))) public rewardAccruedSupply;
    mapping(uint8 => mapping(address => mapping(address => uint256))) public rewardAccruedBorrow;
...
contract RewardDistributor is RewardDistributorStorage, Exponential {
    ...
    function setRewardSpeedInternal(uint8 rewardType, CToken cToken, uint256 newSupplySpeed, uint256 newBorrowSpeed) internal {
        ...
        if (currentRewardSupplySpeed != 0) {
            ...
            RewardMarketState storage supplyState = rewardSupplyState[rewardType][address(cToken)];
        ...
        if (currentRewardBorrowSpeed != 0) {
            ...
            RewardMarketState storage borrowState = rewardBorrowState[rewardType][address(cToken)];
    ...
    function distributeSupplierReward(uint8 rewardType, address cToken, address supplier) internal {
        ...
        uint supplierAccrued = add_(rewardAccruedSupply[rewardType][cToken][supplier], supplierDelta);
        rewardAccruedSupply[rewardType][cToken][supplier] = supplierAccrued;
    ...
    function distributeBorrowerReward(uint8 rewardType, address cToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint borrowerAccrued = add_(rewardAccruedBorrow[rewardType][cToken][borrower], borrowerDelta);
            rewardAccruedBorrow[rewardType][cToken][borrower] = borrowerAccrued;
        ...
        for (uint256 i = 0; i < cTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint256 j = 0; j < holders.length; j++) {
                    ...
                    rewardAccruedBorrow[rewardType][address(cToken)][holders[j]] = grantRewardInternal(
                        rewardType,
                        holders[j],
                        rewardAccruedBorrow[rewardType][address(cToken)][holders[j]]
                    );
                }
            }
            if (suppliers == true) {
                ...
                for (uint256 j = 0; j < holders.length; j++) {
                    ...
                    rewardAccruedSupply[rewardType][address(cToken)][holders[j]] = grantRewardInternal(
                        rewardType,
                        holders[j],
                        rewardAccruedSupply[rewardType][address(cToken)][holders[j]]
                    );
                }
            }
        }
